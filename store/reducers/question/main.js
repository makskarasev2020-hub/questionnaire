...
export default (state = INITIAL_STATE, { type, payload }) => {
    switch (type) {
        ...
        case REMOVE_ANSWERS_NOT_SEND_DATA:
            console.log('REMOVE_ANSWERS_NOT_SEND_DATA')
            console.log(state.answersNotSend[payload.userId ? payload.userId : 'noAuth'])

            let itemsNotSendToRemove =
                state.answersNotSend[payload.userId ? payload.userId : 'noAuth'] || [];
            let filteredArray = itemsNotSendToRemove.filter(item => item.questionnaireId !== payload.questionnaireId);;

            console.log(filteredArray)

            let resNotSendToRemove = {
                ...state.answersNotSend,
                [payload.userId ? payload.userId : 'noAuth']: [
                    ...filteredArray
                ],
            ...
            AsyncStorage.setItem('answersNotSend', JSON.stringify(resNotSendToRemove));

            return {
                ...state,
                answersNotSend: resNotSendToRemove,
            };
